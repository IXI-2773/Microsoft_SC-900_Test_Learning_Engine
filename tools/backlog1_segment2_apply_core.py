...
def replace_function(text: str, name: str, next_name: str | None, replacement: str) -> str:
...
def patch_session_models() -> None:
    path = Path("session_models.py")
    text = path.read_text(encoding="utf-8")
    text = replace_once(
        text,
        "from typing import Any, TypedDict, cast",
        "from typing import Any, NotRequired, TypedDict, cast",
        label="session_models import",
    )
    text = replace_once(
        text,
        "    prediction_snapshot: dict[str, Any]\n\n\nclass QuestionHistoryEvent",
        "    prediction_snapshot: dict[str, Any]\n\n\nclass SessionAnswerState(AnswerState, total=False):\n    question_id: str\n\n\nclass QuestionHistoryEvent",
        label="SessionAnswerState insertion",
    )
    text = replace_once(
        text,
        "    bank_file: str\n    mode: str",
        ...
        label="SessionSnapshot identity fields",
    )
    text = replace_once(
        text,
        "    answers: list[AnswerState]\n",
        "    answers: list[SessionAnswerState]\n",
        label="SessionSnapshot answer type",
    )
...
def patch_session_store() -> None:
    ...
    text = replace_once(
        ...
        label="session identity imports",
    )
    text = replace_once(
        text,
        "    SessionAnswerEvent,\n    SessionSnapshot,",
        "    SessionAnswerEvent,\n    SessionAnswerState,\n    SessionSnapshot,",
        label="SessionAnswerState import",
    )
    text = replace_once(
        text,
        "SESSION_SCHEMA_VERSION = 3\n",
        "SESSION_SCHEMA_VERSION = 4\nLEGACY_SESSION_SCHEMA_VERSION = 3\n",
        label="session schema version",
    )
    ...
    *,
    ...
    coerce_anchor = '''def _coerce_int_list(value: Any, *, field: str, minimum: int = 0) -> list[int]:
    if value in (None, ""):
        return []
...
'''
    ...
    return ids
'''
    text = replace_once(text, coerce_anchor, coerce_replacement, label="canonical ID coercion")
