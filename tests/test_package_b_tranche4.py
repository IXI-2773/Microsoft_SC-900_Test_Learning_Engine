...
class PackageBTranche4RealCandidateClosureTests(unittest.TestCase):
    ...
    def test_58_package_c_inactive(self) -> None:
        ...
        self.assertFalse((REPOSITORY_ROOT / "sc900_bank_v8_length_rebalanced_t5.json").exists())
    ...
    def test_60_no_t5_artifacts(self) -> None:
        evidence = REPOSITORY_ROOT / "content_revision_evidence"
        matches = list(evidence.rglob("*t5*")) + list(evidence.rglob("*T5*"))
        docs = list((REPOSITORY_ROOT / "docs/research/SC900-ANSWER-LENGTH-LEAKAGE-REPAIR-001").glob("*TRANCHE5*"))
